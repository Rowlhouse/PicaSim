...
{
  mColourTop = Vector3(0.3f, 0.3f, 0.5f);
  mColourBottom = Vector3(0.3f, 0.3f, 0.5f);
...
{
  mColourTop =  Vector3(0.5f, 0.5f, 0.5f);
  mColourBottom =  Vector3(0.5f, 0.5f, 0.5f);
  mColourSides =  Vector3(0.5f, 0.5f, 0.5f);
  mColourFront = Vector3(0.5f, 0.5f, 0.5f);
}

//---------------------------------------------------------------------------------------------------------------------
void AeroplaneGraphics::Box::SetRed()
{
  mColourTop = Vector3(1.0f, 0.0f, 0.0f);
  mColourBottom = Vector3(1.0f, 0.0f, 0.0f);
  mColourSides = Vector3(1.0f, 0.0f, 0.0f);
  mColourFront = Vector3(1.0f, 0.0f, 0.0f);
}

//---------------------------------------------------------------------------------------------------------------------
void AeroplaneGraphics::Box::SetBlack()
{
  mColourTop = Vector3(0,0,0);
  mColourBottom = Vector3(0,0,0);
  mColourSides = Vector3(0,0,0);
  mColourFront = Vector3(0,0,0);
}
  ...
  if (numWindStreamers)
  {
    ...
    for (float side = 0 ; side < numSide-0.5f ; ++side)
    {
      ...
      for (int i = 0 ; i != numWindStreamers ; ++i)
      {
        ...
        Vector3 windPos = groundPos + Vector3(0.0f,0.0f,dz);
    ...
    )
  {
    if (PicaSim::GetInstance().GetStatus() == PicaSim::STATUS_FLYING)
    {
      ...
      for (int iPt = 0 ; iPt != numPts ; ++iPt)
      {
        ...
        {
          const Vector3& vel = mAeroplane->GetVelocity();
          RenderManager::GetInstance().GetDebugRenderer().SetGraphProperties(GRAPH_CLIMB_RATE, numPoints, -options.mGraphClimbRate, options.mGraphClimbRate, Vector3(0,0,1));
        ...
        if (options.mGraphWindVerticalVelocity)
        {
          ...
          Vector3 wind = Environment::GetInstance().GetWindAtPosition(aeroplaneTM.GetTrans(), Environment::WIND_TYPE_SMOOTH | Environment::WIND_TYPE_GUSTY);
          float speed = wind.z;
          RenderManager::GetInstance().GetDebugRenderer().SetGraphProperties(GRAPH_WIND_VERTICAL_VELOCITY, numPoints, -options.mGraphWindVerticalVelocity, options.mGraphWindVerticalVelocity, Vector3(1,0,1));
          RenderManager::GetInstance().GetDebugRenderer().AddGraphPoint(GRAPH_WIND_VERTICAL_VELOCITY, speed);
        }
        else
        {
        ...
        {
          RenderManager::GetInstance().GetDebugRenderer().SetGraphProperties(GRAPH_FPS, numPoints, 0.0f, options.mGraphFPS, Vector3(1,1,1));
          RenderManager::GetInstance().GetDebugRenderer().AddGraphPoint(GRAPH_FPS, PicaSim::GetInstance().GetSettings().mStatistics.mFPS);
        }
        else
        {
