...
//---------------------------------------------------------------------------------------------------------------------
uint32_t GetStickVisibleMask(const GameSettings& gs, const Aeroplane* aeroplane)
{
  uint32_t result = 0;
...
void HumanController::UpdateAccelerometer(float deltaTime)
{
  ...
  if (accelerometerEnabled)
  {
    ...
    int32_t iX = s3eAccelerometerGetX();
    int32_t iY = s3eAccelerometerGetY();
    int32_t iZ = s3eAccelerometerGetZ();
...
void HumanController::EntityUpdate(float deltaTime, int entityLevel)
{
  ...
  uint32_t stickVisibleMask = GetStickVisibleMask(mGameSettings, mAeroplane);
...
void HumanController::RenderOverlayUpdate(int renderLevel, DisplayConfig& displayConfig)
{
  ...
  if (controllerColour[3])
  {
    ...
    if (stickVisibleMask & STICK_RIGHT)
      RenderController(displayConfig.mLeft + mRightControllerPosX * displayConfig.mWidth, displayConfig.mBottom + mRightControllerPosY * displayConfig.mHeight, mControllerSizeX, mControllerSizeY, mGameSettings.mOptions.mControllerStyle, controllerShader);
    if (stickVisibleMask & STICK_LEFT)
      RenderController(displayConfig.mLeft + mLeftControllerPosX * displayConfig.mWidth, displayConfig.mBottom + mLeftControllerPosY * displayConfig.mHeight, mControllerSizeX, mControllerSizeY, mGameSettings.mOptions.mControllerStyle, controllerShader);
  }
  if (stickColour[3])
  {
    if (gGLVersion == 1)
    ...
    if (stickVisibleMask & STICK_RIGHT)
    {
      RenderStick(displayConfig.mLeft + mRightControllerPosX * displayConfig.mWidth, displayConfig.mBottom + mRightControllerPosY * displayConfig.mHeight, options.mControllerStickCross,
        mInputControls[GetControlForStick(ControllerSettings::CONTROLLER_RIGHT_HORIZONTAL, options.mControllerMode)], 
        mInputControls[GetControlForStick(ControllerSettings::CONTROLLER_RIGHT_VERTICAL, options.mControllerMode)], mControllerSizeX, mControllerSizeY, s, s, controllerShader);
    }
    if (stickVisibleMask & STICK_LEFT)
    {
      RenderStick(displayConfig.mLeft + mLeftControllerPosX * displayConfig.mWidth, displayConfig.mBottom + mLeftControllerPosY * displayConfig.mHeight, options.mControllerStickCross,
        mInputControls[GetControlForStick(ControllerSettings::CONTROLLER_LEFT_HORIZONTAL, options.mControllerMode)], 
        mInputControls[GetControlForStick(ControllerSettings::CONTROLLER_LEFT_VERTICAL, options.mControllerMode)], mControllerSizeX, mControllerSizeY, s, s, controllerShader);
    }
  }
