  ...
  for (int iZ = 0 ; iZ != wallHeight ; ++iZ)
  {
    for (int iX = 0 ; iX != wallLength ; ++iX)
    {
      tm.t = mInstance->mGameSettings.mEnvironmentSettings.mObserverPosition + 
        ...
        Vector3(r.GetValue(-range, range), r.GetValue(-range, range), 0.0f);
...
int PicaSim::ShowInGameDialog(float width, float height, const char* title, const char* text, const char* button0, const char* button1, const char* button2)
{
  ...
  while (doDialog)
  {
      ...
      )
    {
      ...
      int32_t updateTime = currentTime > lastTime ?
        MIN((int32_t)(currentTime - lastTime), 100) : 0;
  ...
  while (doDialog)
  {
    ...
    while (helpIndex >= 0)
    {
        ...
        )
      {
        ...
        uint64_t currentTime = s3eTimerGetMs();
        int32_t updateTime = currentTime > lastTime ?
          MIN((int32_t)(currentTime - lastTime), 1000) : 0;
        lastTime = currentTime;
        float dt = updateTime * 0.001f;
        totalTime += dt;
...
//---------------------------------------------------------------------------------------------------------------------
PicaSim::UpdateResult PicaSim::Update(int64_t deltaTimeMs)
{
  ...
  if (mSettingsMenuOverlay->IsPressed())
  {
    ...
    if (actions.mReloadTerrain)
    {
      ...
      //mPlayerAeroplane->Init( mGameSettings.mAeroplaneSettings, &mInstance->mObserver->GetCameraTransform((void*) 0).GetTrans(), loadingScreen);
      Vector3 var = mInstance->mObserver->GetCameraTransform((void*) 0).GetTrans();
      mPlayerAeroplane->Init( mGameSettings.mAeroplaneSettings, &var, loadingScreen);
      // Reset any challenge
