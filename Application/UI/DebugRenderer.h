...
private:
  struct Point
  {
    Point(const Vector3& pos, float size, const Vector3& colour) : mColour(colour), mSize(size) {mTM.SetIdentity(); mTM.SetTrans(pos);}
    Point(const Transform& tm, float size, const Vector3& colour) : mTM(tm), mColour(colour), mSize(size) {}
    Transform mTM;
    Vector3 mColour;
    float mSize;
  };
