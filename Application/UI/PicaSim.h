...
public:
  ...
  UpdateResult Update(int64_t deltaTimeMs);

  Mode GetMode() const {return mMode;}
  void SetMode(Mode mode) {mMode = mode;}

  StatusFly GetStatus() const {return mStatus;}
  void SetStatus(StatusFly status) {mStatus = status;}
