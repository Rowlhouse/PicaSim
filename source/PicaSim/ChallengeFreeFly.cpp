...
void ChallengeFreeFly::GxRender(int renderLevel, DisplayConfig& displayConfig)
{
  ...
  if (PicaSim::GetInstance().GetStatus() == PicaSim::STATUS_FLYING && options.mFreeFlightTextAtTop)
    textRect = CIwRect(displayConfig.mLeft, displayConfig.mBottom + fontHeight*1/4,(int16)displayConfig.mWidth,fontHeight);
  else
    textRect = CIwRect(displayConfig.mLeft,(int16)(displayConfig.mBottom + displayConfig.mHeight - fontHeight*5/4),(int16)displayConfig.mWidth,fontHeight);
  ...
  if (options.mFreeFlightTextBackgroundOpacity > 0.0f)
  {
    ...
    uint32 x = strlen(txt) * fontHeight * 1 / 2;
    uint32 x0 = (w - x) / 2;
  ...
  {
    IwGxFontSetRect(CIwRect(displayConfig.mLeft,(int16)(displayConfig.mBottom + displayConfig.mHeight - fontHeight*5/4),(int16)displayConfig.mWidth,fontHeight));
