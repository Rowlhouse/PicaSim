...
//---------------------------------------------------------------------------------------------------------------------
int32 Menu::Update(bool keepAwake, bool throttle, bool& resourcesHaveChanged, const GameSettings& gameSettings)
{
  ...
  int32 updateTime = currentTime > mLastTime ?
    MIN((int32)(currentTime - mLastTime), 1000) : 0;
...
Texture* AddCentredImage(const char* filename, bool use16Bit, CIwUIImage** image)
{
  ...
  int16 imageWidth  = texture->GetWidth();
  int16 imageHeight = texture->GetHeight();
  ...
  if (h < height)
  {
    ...
    uv0.x = (int16) (f * imageWidth);
    uv0.y = 0;
    uv1.x = (int16) ((1.0f - f) * imageWidth);
    uv1.y = imageHeight;
  }
  else
  {
    float f = 0.5f * (1.0f - height/h);
    uv0.y = (int16) (f * imageHeight);
    uv0.x = 0;
    uv1.y = (int16) ((1.0f - f) * imageHeight);
    uv1.x = imageWidth;
  }
  uv0.x = ClampToRange(uv0.x, int16(0), imageWidth);
  uv1.x = ClampToRange(uv1.x, int16(0), imageWidth);
  uv0.y = ClampToRange(uv0.y, int16(0), imageHeight);
  uv1.y = ClampToRange(uv1.y, int16(0), imageHeight);
